// API Base URL
        ...
        function showSection(sectionId) {
            ...
            if (sectionId === 'patient-log') {
                ...
                loadUsers();
            } else if (sectionId === 'financial-tracking') {
                renderFinancialTracking();
            }
        }
...
async function renderFinancialTracking() {
    const container = document.getElementById('financial-tracking-content');
    if (!currentUser) return;
    // Patient selector
    let patientOptions = allPatients.map(p => `<option value="${p.id}">${p.patient_number} - ${p.first_name} ${p.last_name}</option>`).join('');
    if (!patientOptions) {
        container.innerHTML = '<div style="color:#e74c3c;">No patients found. Add a patient first.</div>';
        return;
    }
    ...
    container.innerHTML = `
        <form id="patientRevenueForm" style="margin-bottom:24px; display:flex; align-items:center; gap:20px;">
            <label><strong>Patient:</strong></label>
            <select id="revenuePatientSelect" required style="min-width:220px;">${patientOptions}</select>
            <label><strong>Month:</strong></label>
            <input type="month" id="revenueMonthSelect" value="${defaultMonth}" required style="min-width:140px;" />
            <button type="submit" class="btn btn-small">Get Revenue</button>
        </form>
        <div id="patientRevenueResult" style="margin-bottom:30px;"></div>
        <div id="addRevenueFormContainer"></div>
    `;
    document.getElementById('patientRevenueForm').addEventListener('submit', async function(e) {
        e.preventDefault();
        const patientId = document.getElementById('revenuePatientSelect').value;
        const monthYear = document.getElementById('revenueMonthSelect').value;
        await showPatientMonthlyRevenue(patientId, monthYear);
        await renderAddRevenueForm(patientId, monthYear);
    });
    // Auto-load for first patient/month
    await showPatientMonthlyRevenue(allPatients[0].id, defaultMonth);
    await renderAddRevenueForm(allPatients[0].id, defaultMonth);
}

async function renderAddRevenueForm(patientId, monthYear) {
    const container = document.getElementById('addRevenueFormContainer');
    // Only admins can add/edit
    if (currentUser.role !== 'admin') {
        container.innerHTML = '';
        return;
    }
    // Fetch record for this patient/month
    let record = null;
    try {
        const resp = await authenticatedFetch(`${API_BASE}/patients/${patientId}/financial`);
        if (resp && resp.ok) {
            const records = await resp.json();
            record = records.find(r => r.month_year === monthYear);
        }
    } catch {}
    container.innerHTML = `
        <form id="addRevenueForm" style="background:#f8f9fa; border-radius:8px; padding:18px 22px; box-shadow:0 2px 8px #0001; margin-bottom:18px; max-width:500px;">
            <h4 style="margin-top:0;">${record ? 'Update' : 'Add'} Revenue for this Patient (${monthYear})</h4>
            <div class="form-group">
                <label>Monthly Revenue ($)</label>
                <input type="number" id="revenueInput" name="monthly_revenue" min="0" step="0.01" value="${record ? record.monthly_revenue : ''}" required />
            </div>
            <div class="form-group">
                <label>Sessions Attended</label>
                <input type="number" id="sessionsInput" name="sessions_attended" min="0" value="${record ? record.sessions_attended : ''}" required />
            </div>
            <div class="form-group">
                <label>Notes</label>
                <input type="text" id="notesInput" name="notes" value="${record ? (record.notes || '') : ''}" />
            </div>
            <button type="submit" class="btn">${record ? 'Update' : 'Add'} Record</button>
            <div id="addRevenueAlert" style="margin-top:10px;"></div>
        </form>
    `;
    document.getElementById('addRevenueForm').addEventListener('submit', async function(e) {
        e.preventDefault();
        const revenue = parseFloat(document.getElementById('revenueInput').value);
        const sessions = parseInt(document.getElementById('sessionsInput').value);
        const notes = document.getElementById('notesInput').value;
        const alertDiv = document.getElementById('addRevenueAlert');
        try {
            let resp;
            if (record) {
                // Update
                resp = await authenticatedFetch(`${API_BASE}/patients/${patientId}/financial/${record.id}`, {
                    method: 'PUT',
                    body: JSON.stringify({ monthly_revenue: revenue, sessions_attended: sessions, notes })
                });
            } else {
                // Create
                resp = await authenticatedFetch(`${API_BASE}/patients/${patientId}/financial`, {
                    method: 'POST',
                    body: JSON.stringify({ patient_id: patientId, month_year: monthYear, monthly_revenue: revenue, sessions_attended: sessions, notes })
                });
            }
            if (resp && resp.ok) {
                alertDiv.innerHTML = '<span style="color:green;">Saved!</span>';
                await showPatientMonthlyRevenue(patientId, monthYear);
            } else if (resp) {
                const err = await resp.json();
                alertDiv.innerHTML = `<span style='color:#e74c3c;'>${err.detail || 'Error saving record.'}</span>`;
            }
        } catch (err) {
            alertDiv.innerHTML = '<span style="color:#e74c3c;">Error saving record.</span>';
        }
    });
}

async function showPatientMonthlyRevenue(patientId, monthYear) {
    const resultDiv = document.getElementById('patientRevenueResult');
    ...
    try {
        ...
        if (resp && resp.ok) {
            ...
            if (record) {
                resultDiv.innerHTML = `<div style='font-size:1.1em;'><strong>Revenue for this patient in ${monthYear}:</strong> <span style='color:#27ae60;'>$${parseFloat(record.monthly_revenue).toLocaleString(undefined, {minimumFractionDigits:2})}</span></div>`;
            } else {
                resultDiv.innerHTML = `<div style='color:#888;'>No revenue record for this patient in ${monthYear}.</div>`;
            }
        } else {
            resultDiv.innerHTML = '<div style="color:#e74c3c;">Error loading record.</div>';
        }
    } catch {
        resultDiv.innerHTML = '<div style="color:#e74c3c;">Error loading record.</div>';
    }
